...
\end{equation}
shown with two general vectors $\bm{u}$ and $\bm{v}$ and $\backslash$ as the left matrix divide operator. For this we used that we can rewrite $(\bm{LL^{\intercal}})^{-1} \bm{v}$ as a solution $\bm{x}$ of a linear system of the form $\bm{LL^{\intercal}} \bm{x}=\bm{v}$, which we now solve for $\bm{x}$. This is a very stabel and efficient way of calculating the equation. The implementation of the log marginal likelihood is shown in~\ref{code:mll}.\\
\lstinputlisting[firstline=109,lastline=117,caption={Log marginal likelihood}, label={code:mll}]{../PI_GP_regressor/main_class.py}
...
\lstinputlisting[firstline=247,lastline=263,caption={Predictive distribution}, label={code:predictive_distribution}]{../PI_GP_regressor/main_class.py}
