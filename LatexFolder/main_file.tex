...
A very, if not the most important part of the process is the optimization of the hyperparameters. The optimization is done by maximizing the marginal log likelihood (mll) of the joint GP with respect to the hyperparameter. The mll is defined as:
\begin{equation}
...
\end{equation}
with $\bm{K}$ being the covariance matrix of the joint GP in equation~\ref{eq:jointGP_observations}. For the optimization we used different gradient based optimizers, aswell as a heuristic search method, namely the Nelder-Mead method. All the optimizers used came from the scipy.optimize.minimize function from the scipy library~\cite{scipy}.
The optimization of the hyperparameters deemed to be harder than expected.
Different optimizers were used. Mention that the results varie (numerical
instabilities, etc.) but with sufficient number of optimization runs the values
always reach nearly the same number of magintude. gradient based optimizers fail with increasing number of training points. However, the  the Nelder-Mead optimizer still performs well with a high number of training points.   
...
three main types of PDE, namely Elliptic, Hyperbolic, and Parabolic. The
