...
\lstinputlisting[firstline=97,lastline=113,caption={Analytical computation of the derivatives}, label={code:kernel_analytical}, breaklines = False]{../PI_GP_regressor/kernels/kernel_oscillator_3variables.py}
...
\lstinputlisting[firstline=111,lastline=119,caption={Log marginal likelihood}, label={code:mll}]{../PI_GP_regressor/main_class.py}
...
\lstinputlisting[firstline=257,lastline=273,caption={Predictive distribution}, label={code:predictive_distribution}]{../PI_GP_regressor/main_class.py}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
with $\alpha$ as the thermal diffusivity. Here we use the same forcing term $f(x,t) = e^{-t}(4\pi^2-1)\sin{(2\pi x)}$, parameter $\alpha = 1$ and boundary conditions as were used in \cite{RAISSI} in order to compare the accuracy of our implementation. As described before, the overall implementation is basically the same, so we should expect similar results. The ground truth was calculated for a grid of size 100x100. The results can be seen in figure \ref{fig:results_heatequation}. The model managed to predict the ground truth very accurately with relative $L^2$ errors of $L^2_u = 0.00381 $ and $L^2_f = 0.00804$. The predictions only have small errors on the domain edges and are nearly error free in the central part of the grid. The jitter applied to the diagonal elements of the gram matrix $K$ was set to $10^{-6}$. With a jitter value in the range of $10^{-7}$, smaller optimization minimas could be found with slightly smaller $L_2$ errors. However, the predictive variance would be smaller than zero (numerical errors) in some central regions and the optimization landscape would get increasingly rough.
\begin{figure}
    \centering
...
The relative $L^2$ error for the prediction of $u(x,y,t)$ was 0.0163 and $c$ was optimized to $c = 1.0081$. To further increase the accuracy we added extra training points for $u$ on the boundary of the domain. In figure \ref{fig:training_points_wave} the distribution of the training points is shown. This is a very straight forward way of increasing the accuracy. Specifically for boundary value problems, data on the domain edges can be easily accessible. For example, one could look at the example above as a membrane, which is fixed on a square. Therefore, the edges would be clamped down and the amplitude would be zero. With the incorporation of 320 training points on the boundary of $u$, the relative error of the predictive mean would decrease to a value of $L^2_u = 0.00355$. The hyperparameter $c$ was optimized to $c = 0.9974$. We can observe, that the accuracy of the hyperparameter did not increase . However, the overall accuracy of the prediction was improved.

%0.016 400 data points no boundary points
\begin{figure}
    \centering
