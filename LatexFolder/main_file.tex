...
\lstinputlisting[firstline=22,lastline=37,caption={Computation of the derivatives using autograd}, label={code:kernel_autograds}, breaklines = False]{../PI_GP_regressor/kernels/kernel_oscillator_3variables.py}
...
% \begin{align}
%     \bm{u^{\intercal}} (\bm{LL^{\intercal}})^{-1} \bm{v}\\
%     \alpha = (\bm{LL^{\intercal}})^{-1} \bm{v}\\
% \end{align}
% We can now rewrite this as:
% \begin{align}
%     \bm{y} = \bm{LL^{\intercal}} \rightarrow \bm{\alpha} = \bm{L^{\intercal} /(L / y)}
% \end{align}
...
\lstinputlisting[firstline=113,lastline=121,caption={Log marginal likelihood}, label={code:mll}]{../PI_GP_regressor/main_class.py}
...
\lstinputlisting[firstline=315,lastline=351,caption={Predictive distribution}, label={code:predictive_distribution}]{../PI_GP_regressor/main_class.py}
\newpage
\section{Results}
