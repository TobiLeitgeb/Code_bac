...
In our last example we will look at a case where we do not incorporate additional ODE parameters $\phi$ in the form of hyperparameters into the kernel. This is to look at the general predictive performance of the informed model. The informed, aswell as the vanilla model will both not incorporate ARD, therefore there will be one singel length scale $l$ for both spatial dimensions.\\
\\
...
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{../final_examples/poisson/predictive_mean.png}
    \caption{}
    ~\label{fig:poisson}
\end{figure}
The most notable difference between the informed and the vanilla model are the predictions at the boundary of the domain. The vanilla model fails to correctly predict the boundaries, however the informed model manages to predict the boundaries very well. Overall, the performance of the infomed model is significantly better. The relative error for $u$ is and $f$ is. Comparing these values to the vanilla model with relative errors of $u$ and $f$ we can see that the informed model is more than an order of magnitude more accurate. This is also reflected in the MSEs. The MSEs for the informed model are $MSE_u = 1.419 \cdot 10^{-7}$ and $MSE_f = 4.609 \cdot 10^{-3}$. The MSEs for the vanilla model are $MSE_{u,va} = 5.469 \cdot 10^{-4}$ and $MSE_{f,va} = 2.692$.\\
\\
