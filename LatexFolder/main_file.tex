...
In this section we will present the computiational implementation in python of the kernel, the log marginal likelihood and of the predictive distribution. For greatly improving the computation time the jax library was used in order to use the vectorization and jit.
\subsection{Kernel}
...
\end{aligned}
\end{equation}
 with $k_{uu} = \sigma_f^2  \exp\left({-\frac{0.5}{l^2} {(t-t')}^2}\right)$ beeing the general SE kernel introduced in section \ref{subsec: Kernels}. However, this can be done with any other kernel aswell. The analytical computation of the derivatives is very simple for the SE kernel, but can get more complicated for other kernels. 
\lstinputlisting[firstline=99,lastline=115,caption={Analytical computation of the derivatives},label={code:kernel_analytical}]{../PI_GP_regressor/kernels/kernel_oscillator_3variables.py}
\lstinputlisting[firstline=17,lastline=33,caption={Computation of the derivatives using autograd},label={code:kernel_autograds}]{../PI_GP_regressor/kernels/kernel_oscillator_3variables.py}
