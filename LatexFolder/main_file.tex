...
\end{aligned}
\end{equation}
 with $k_{uu} = \sigma_f^2  \exp\left({-\frac{0.5}{l^2} {(t-t')}^2}\right)$ beeing the general SE kernel introduced in section \ref{subsec: Kernels}. Note that this can be done with any other kernel aswell. The analytical computation is shown in~\ref{code:kernel_analytical}.\\
\lstinputlisting[firstline=99,lastline=115,caption={Analytical computation of the derivatives},label={code:kernel_analytical},breaklines = False]{../PI_GP_regressor/kernels/kernel_oscillator_3variables.py}
For the SE kernel the computations are rather simple and can easily be done analytically. However, if one chooses a complex kernel, the analytical computation can become rather complicated an time consuming. Therefore, we also implemented the the kernel derivatives with help of autograd. Namely, we used the jax library for the automatic differentiation. The code for the autograd computation of the derivatives is shown in \ref{code:kernel_autograds}. The big advantage of this approach is that it is sufficient to do the calculations as done in~\ref{eq:kernel_analytical} and implement it as shown in the code. With this the choice of the general kernel $k_{uu}$ is only restricted within the jax library. This means that the kernel can be any function that is traceable by the jax autograd an jit functions. 
\lstinputlisting[firstline=17,lastline=33,caption={Computation of the derivatives using autograd},label={code:kernel_autograds},breaklines = False]{../PI_GP_regressor/kernels/kernel_oscillator_3variables.py}
...
A general problem of Gaussian processes is the numerical calculation of the inverse of the covariance matrix $\bm{K}$. This problem becomes increasingly apperent with a high number of training points. Especially in the training process the inverse of $\bm{K}$ has to be calculated several times. However, with the vectorized implementation aswell as the use of just in time (jit) compiled function from the jax library the computation time was not a major problem. Additionaly, the number of training points was held at a rather low number (highest number was 400 for the 3d wave equation). Still, the calculation of the inverse can be a source of numerical instabilities. To minimized numerical errors and instabilities the inverse was calculated using the Cholesky decomposition similar to the implementation in~\cite{RasmussenCarlEdward}.\\
The Cholesky decomposition of a positive definite, symmetric matrix $\bm{A}$ is given by:
\begin{equation}
    \bm{K} = \bm{L}\bm{L}^\intercal
\end{equation}
with $\bm{L}$ being a lower triangular matrix. With this the the general term involving the inversion of $\bm{K}$ can be written as:
\begin{equation}
    \bm{u}^\intercal \bm{K}^{-1}\bm{v} = \bm{u}^\intercal (\bm{LL^{\intercal}})^{-1} \bm{v} = \bm{u}^\intercal \bm{L^{\intercal} \backslash (L\backslash v)}
\end{equation}
shown with two general vectors $\bm{u}$ and $\bm{v}$ and $\backslash$ as the left matrix divide operator. This can be solved by first solving the linear system $\bm{L}\bm{v} = \bm{u}$ and then solving the linear system $\bm{L}^\intercal \bm{z} = \bm{y}$. This way of calculating the inverse of $\bm{K}$ is more stable than the direct inversion of $\bm{K}$. The implementation of the log marginal likelihood is shown in~\ref{code:mll}.\\
\lstinputlisting[firstline=109,lastline=117,caption={Log marginal likelihood}]{../PI_GP_regressor/main_class.py}
...
\lstinputlisting[firstline=247,lastline=263,caption={Predictive distribution}]{../PI_GP_regressor/main_class.py}
