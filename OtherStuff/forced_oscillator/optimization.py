...
def log_marginal_likelihood_to_optimize(Kernel:callable,  X, Y, targets, noise):
    ...
    def function_to_optimize(hyperparams):
        mll = log_marginal_likelihood(Kernel, X, Y, targets,noise, hyperparams)
        return mll
    return function_to_optimize

def optimization_restarts_parallel_LBFGS(Kernel: callable, n_restarts: int, n_threads: int,opt_dictionary: dict, X, Y, targets, noise) -> dict:
    """
    ...
    """
    def single_optimization_run(Kernel, X, Y, targets, noise):
        ...
        res = minimize(log_marginal_likelihood_to_optimize(Kernel, X, Y, targets,noise), x0=theta_initial,
                       method='L-BFGS-B', bounds=opt_dictionary['bounds'],
                       options={'gtol': opt_dictionary['gtol']})
        return res

    
    results = Parallel(n_jobs=n_threads)(delayed(single_optimization_run)(Kernel, X, Y, targets,noise) for _ in tqdm(range(n_restarts)))
...
def grad_log_marginal_likelihood(Kernel:callable, X, Y, targets, noise:list):
    return jacfwd(log_marginal_likelihood_to_optimize(Kernel, X, Y, targets, noise))


def optimization_restarts_parallel_CG(Kernel: callable, n_restarts: int, n_threads: int,opt_dictionary: dict, X, Y, targets, noise) -> dict:
    """
...
def gaussian_optimization(dictionary: dict, Kernel,restarts, X, Y, targets, noise):
 
    
    def objective_function(params):
        l_x, sigma_f_sq, l_t, alpha = params
        return log_marginal_likelihood(Kernel, X, Y, targets, noise, [l_x, sigma_f_sq, l_t, alpha]).item()  # we minimize -log_likelihood
    best_resulty = np.inf
    best_result = None
    for i in tqdm(range(restarts)):
        i = i*np.random.randint(100)
        try:
            result = gp_minimize(objective_function,  
                                **dictionary,
                                random_state=i)
            if result.fun < best_resulty:
                best_resulty = result.fun
                best_result = result
        except:
            print("skipped")
            pass
    if best_result is None:
        print("went into method again")
        best_result = gaussian_optimization(dictionary, Kernel,restarts, X, Y, targets, noise)
    print("------BO------")
    print(best_result.fun, best_result.x)
    return best_result

def optimization_restarts_parallel_TNC(Kernel: callable, n_restarts: int, n_threads: int,opt_dictionary: dict, X, Y, targets, noise) -> dict:
    """
    performs the optimization of the hyperparameters in parallel and returns the best result.
    n_restarts: number of restarts of the optimization
    n_threads: number of threads to use for the parallelization
    ...
    def single_optimization_run(Kernel, X, Y, targets, noise):
        ...
        theta_initial = opt_dictionary['theta_initial']()
        res = minimize(log_marginal_likelihood_to_optimize(Kernel, X, Y, targets,noise), x0=theta_initial,
                       method='TNC', jac=jit(grad_log_marginal_likelihood(Kernel, X, Y, targets,noise)),bounds=opt_dictionary['bounds'])
        return res
